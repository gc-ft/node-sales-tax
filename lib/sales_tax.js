...
if (fetchApi === undefined && typeof global !== 'undefined' && global.fetch) {
  fetchApi = global.fetch;
}
var fetch = !fetchApi ? require('cross-fetch') : fetchApi;
var check_fraud_eu_vat = require("validate-vat-ts");
...
SalesTax.prototype.__buildSalesTaxContext = function(
  ...
  // Compute full tax rate (country + state)
  // Notice: convert the result to a fixed value, then parse it again as a \
  //   floating point number to correct floating point approximations due to \
  //   the JS VM. This fixes eg. (0.21 - 0.14) = 0.06999999999999998 <> O.07 \
  //   miscalculations. We support up to 6 floating point decimals here.
  var fullRate = parseFloat((countryTax.rate + stateTax.rate).toFixed(6));

  // Generate tax type (multiple sales tax may apply, eg. country + state)
  // Notice: replace the country tax type with the state tax type in the event \
  //   that the state tax is substractive and not additive.
  var type = countryTax.type;

  if (stateTax.rate > 0) {
    if (countryTax.rate > 0) {
      if (type !== stateTax.type) {
